...
var restoreGalleries = function(data){  
    $('.gallery_head h2').text('Galleries');
    i = 0;
    console.log(albumArray);    
    console.log('restoreGalleries working');
    stopLoad = false;
    $('.photos').empty();
    $('.buttonDiv').empty();
    ...
    if (stopLoad != true){
        data.forEach(function(album){
            ...
            $('.gallery_item_container:last').append('<a href="#photos"><div class="col-sm-12 gallery_item"></div></a>');
            $('.gallery_item:last').append('<div class="gallery_img"></div>');
            $('.gallery_item:last').attr('name', album.id);
            $gallery_img = $('.gallery_img');
            $gallery_img_last = $('.gallery_img:last');
            $gallery_img_last.css('background-image', 'url(' + album.photos[0].source +')')
            ...
            $('.gallery_item:last').click(function(){
                var albumName = $(this).children('h4').context.innerText;
                $('.photos').empty();
                if (album.name === 'Timeline Photos'){
                    populatePhotosHelper(galleryData[$(this).attr('name')], 'Miscellaneous');
                } else {
                    populatePhotosHelper(galleryData[$(this).attr('name')], albumName);
                }
            })
            
            if (albumArray[i].name === 'Timeline Photos'){
              $('.gallery_text:last').append('<h4>' + 'Miscellaneous' + '</h4>');
            } else {
              $('.gallery_text:last').append('<h4>' + albumArray[i].name + '</h4>');
            }
        ...
        if (galleryArray.length < albumArray.length){
            i = galleryArray.length;
            getGalleries(pageId, albumArray[i]);
        }
    }
    // FILL GALLERY USING GALLERYDATA (use code similar to getGalleries)
    // FIGURE OUT HOW TO DEAL WITH GALLERIES NOT FULLY LOADING
}
        ...
        data: albumData
          ...
          if (stopLoad != true){
              ...
              $('.gallery_item:last').click(function(){
                  ...
                  $('.photos').empty();
                  //$('.gallery_head h2').empty();
                  if (album.name === 'Timeline Photos'){
                    populatePhotosHelper(galleryData[$(this).attr('name')], 'Miscellaneous');
                  } else {
                    populatePhotosHelper(galleryData[$(this).attr('name')], album.name);
                  }
          ...
          alert("Could not finish loading galleries. Please try again later.");
          console.log(data);
      });
}
...
var populatePhotosHelper = function(data, name){
    console.log(data);
    $photo_container = $('.photos');
    $('.gallery_head h2').text(name);
    ...
    data.photos.forEach(function(photo){
      ...
      $photo_last.click(function(){
        populatePhotoModal(photo.source);
      })
    })    
    $('.buttonDiv').append('<a href="#photos"><button class="btn btn-default btn-lg submit return"><strong>Return to Galleries</strong></button></a>');
    $('.return').click(function(){
        console.log(galleryArray);
        restoreGalleries(galleryArray);
    })            
}
